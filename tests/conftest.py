...
def new_members_csv():
    inputs = NamedTemporaryFile(delete=False)
    inputs.write(b'''email_address,vip,interests.1234abc,interests.abc1234,status,email_type_option,merge_fields.*|FNAME|*,list_id
robin@keboola.com,true,true,true,subscribed,true,Robin,12345
foo@bar.com,false,true,false,pending,false,,12345''')
    inputs.close()
    return inputs
